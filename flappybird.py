...
def main():
    ...
    stream = pyaud.open(format = pyaudio.paInt16, channels = 1, rate = 44100, input_device_index = PYAUDIO_DEVICE_INDEX, input = True)
    ...
    while repeat == True:
        ...
        score_font = pygame.font.SysFont(None, 32, bold=True)  # default font (score)
        end_game_font = pygame.font.SysFont(None, 50, bold=True) #default font (end game score)
        play_on_font = pygame.font.SysFont(None, 40, bold=True)
        images = load_images()
        ...
        while not done:
            ...
            for event in pygame.event.get():
                if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                    ...
                    break
                elif event.type == KEYUP and event.key in (K_PAUSE, K_p, K_SPACE):
                    paused = not paused
                elif event.type == MOUSEBUTTONUP or (event.type == KEYUP and
                        event.key in (K_UP, K_RETURN)):
        ...
        end_game_surface = end_game_font.render('Game over! Score: %i' % score, True, (0, 0, 0))
        end_x = WIN_WIDTH/2 - end_game_surface.get_width()/2
        end_y = WIN_HEIGHT/2 - end_game_surface.get_height()/2
        display_surface.blit(end_game_surface, (end_x, end_y))
        pygame.display.flip()

        pygame.time.wait(500)

        play_on_surface = play_on_font.render("Press SPACE to play again or Q to quit", True, (0,0, 0))
        play_on_x = 20
        play_on_y = end_y + 40
        display_surface.blit(play_on_surface, (play_on_x, play_on_y))

        pygame.display.flip()

        pygame.event.clear()

        #check to continue the program or quit it
        inputted = False
        while not inputted:
            for event in pygame.event.get():
...
if __name__ == '__main__':
    # If this module were to be imported, __name__ would be 'flappybird'.
    # It was executed so call main.
    main()
